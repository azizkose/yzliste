...
export default function TrustStrip() {
  return (
    <div className="border-b border-rd-neutral-200 bg-rd-neutral-50">
        ...
        {TRUST_STRIP_ITEMS.map((item, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-1.5">
              {Icon && (
                <Icon size={13} strokeWidth={2} className="text-rd-neutral-400" aria-hidden="true" />
              )}
              <span className="text-xs text-rd-neutral-500">{item.label}</span>
            </div>
          )
        })}
      </div>
