...
function SkorBari({ skor, oneriler, ucretsizRevizeKullanildi, onUcretsizRevize }: {
  ...
  const { renk, bg, metin } = skor >= 75
    ? { renk: "bg-rd-success-700", bg: "bg-rd-success-50 border-rd-success-700/20", metin: "Harika listing! Pazaryerine hazır." }
    : skor >= 50
    ? { renk: "bg-rd-warning-700", bg: "bg-rd-warning-50 border-rd-warning-700/20", metin: "İyileştirilebilir" }
    : { renk: "bg-rd-danger-700", bg: "bg-rd-danger-50 border-rd-danger-700/20", metin: "Eksik bilgi çok fazla" };

  const skorRenk = skor >= 75 ? "text-rd-success-700" : skor >= 50 ? "text-rd-warning-700" : "text-rd-danger-700";
    ...
    <div className={`border rounded-lg p-4 space-y-3 ${bg}`}>
      ...
      {skor < 75 && oneriler.length > 0 && (
        <div className="space-y-1">
          {oneriler.map((o, i) => (
            <p key={i} className={`text-xs flex items-start gap-1.5 ${skor >= 50 ? "text-rd-warning-700" : "text-rd-danger-700"}`}>
      ...
      {skor < 75 && !ucretsizRevizeKullanildi && (
        <button
          ...
          className={`text-xs font-medium px-3 py-1.5 rounded-lg border transition-colors ${skor >= 50 ? "bg-rd-warning-50 border-rd-warning-700/30 text-rd-warning-700 hover:bg-rd-warning-50" : "bg-rd-danger-50 border-rd-danger-700/30 text-rd-danger-700 hover:bg-rd-danger-50"}`}
        >
      ...
      {gelismisAcik && (
        <div className="mt-3 space-y-3">
          <div>
            ...
            <select value={hedefKitle} onChange={(e) => setHedefKitle(e.target.value)} className="w-full border border-rd-neutral-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rd-primary-800/20 focus:border-rd-primary-800">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(["butce", "orta", "premium"] as const).map((seg) => (
                <button key={seg} type="button" onClick={() => setFiyatSegmenti(seg)}
                  className={`py-2 rounded-lg border text-xs font-medium transition-all ${fiyatSegmenti === seg ? "border-rd-primary-800 bg-rd-primary-100 text-rd-primary-800" : "border-rd-neutral-200 text-rd-neutral-600 hover:border-rd-primary-800"}`}>
    ...
    <div className="space-y-4">
      ...
      {topluHata && (
        <div className="bg-rd-danger-50 border border-rd-danger-700/20 rounded-lg p-4 flex items-center justify-between gap-3">
          <p className="text-sm text-rd-danger-700">{topluHata}</p>
          <button onClick={() => setTopluHata(null)} className="text-rd-danger-700/60 hover:text-rd-danger-700">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={i} className="bg-rd-neutral-100 rounded-lg border border-rd-neutral-200 p-3">
                <kart.Ikon size={18} strokeWidth={1.5} className="text-rd-neutral-400 mb-1.5" />
                <p className="text-xs font-medium text-rd-neutral-900 mb-1">{kart.baslik}</p>
                <p className="text-xs text-rd-neutral-600">{kart.icerik}</p>
              </div>
            ))}
          </div>

          <div className="bg-rd-primary-50 border border-rd-primary-200 rounded-lg p-4">
            <p className="text-xs font-medium text-rd-primary-800 mb-2 flex items-center gap-1.5">
              <ClipboardList size={13} strokeWidth={1.5} />
              Dosyanda ne olmalı?
            </p>
            <p className="text-xs text-rd-neutral-600 mb-2">Sütun adları Türkçe veya İngilizce olabilir — sistem otomatik algılar.</p>
            <div className="grid grid-cols-2 gap-x-4 gap-y-1">
              {[["Ürün Adı / Product Name", "zorunlu"], ["Kategori / Category", "önerilir"], ["Açıklama / Description", "önerilir"], ["Marka / Brand", "isteğe bağlı"]].map(([alan, durum]) => (
                <div key={alan} className="flex items-center gap-1.5 text-xs text-rd-neutral-600">
                  <span className={durum === "zorunlu" ? "text-rd-success-700 font-medium" : durum === "önerilir" ? "text-rd-primary-800" : "text-rd-neutral-200"}>•</span>
                  <span>{alan}</span>
                  <span className={`ml-auto text-[10px] ${durum === "zorunlu" ? "text-rd-success-700 font-medium" : "text-rd-neutral-400"}`}>{durum}</span>
                </div>
              ))}
            </div>
          </div>

          <div className="border border-rd-neutral-200 rounded-lg p-6 text-center">
            <FolderOpen size={32} strokeWidth={1.5} className="text-rd-neutral-400 mx-auto mb-3" />
            <p className="text-sm font-medium text-rd-neutral-900 mb-1">Excel veya CSV yükle</p>
            <p className="text-xs text-rd-neutral-600 mb-4">Şablon gerekmez — sütunlar otomatik algılanır</p>
            <button
              onClick={() => dosyaRef.current?.click()}
              className="bg-rd-primary-800 hover:bg-rd-primary-900 text-white font-medium px-6 py-2.5 rounded-lg transition-colors text-sm"
            >
              Dosya seç
            </button>
            <input ref={dosyaRef} type="file" accept=".xlsx,.xls,.csv" className="hidden" onChange={topluDosyaSec} />
            <p className="text-xs text-rd-neutral-400 mt-3">Desteklenen formatlar: .xlsx, .xls, .csv</p>
          </div>
        </div>
      )}
      ...
      {topluAdim === "onizleme" && topluParse && (
        <div className="space-y-3">
          <div className="border border-rd-neutral-200 rounded-lg p-4">
            <p className="text-sm font-medium text-rd-neutral-900 mb-3">Dosya analizi</p>
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl font-medium text-rd-primary-800">{topluParse.toplam}</span>
              <span className="text-sm text-rd-neutral-600">ürün bulundu</span>
            </div>
            <div className="space-y-1.5">
              {topluParse.kolonlar.map((k) => (
                <div key={k.hedef} className="flex items-center gap-2 text-sm">
                  <Check size={13} strokeWidth={2} className="text-rd-success-700" />
                  <span className="font-medium text-rd-neutral-600">{k.etiket}</span>
                  <span className="text-rd-neutral-400">← &quot;{k.kaynak}&quot;</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="border border-rd-neutral-200 rounded-lg p-4">
            <p className="text-xs font-medium text-rd-neutral-600 mb-2">Platform</p>
            <div className="grid grid-cols-3 gap-2">
              {(Object.keys(TOPLU_PLATFORM_ETIKET) as TopluPlatform[]).map((p) => (
                <button key={p} onClick={() => setTopluPlatform(p)}
                  className={`py-2 px-2 rounded-lg text-xs font-medium border transition-all ${topluPlatform === p ? "border-rd-primary-800 bg-rd-primary-50 text-rd-primary-800" : "border-rd-neutral-200 text-rd-neutral-600 hover:border-rd-primary-800/40"}`}>
          ...
          <div className="border border-rd-neutral-200 rounded-lg p-4">
            <p className="text-xs font-medium text-rd-neutral-600 mb-2">Metin tonu</p>
            <div className="grid grid-cols-3 gap-2">
              {[{ id: "samimi", label: "Samimi" }, { id: "profesyonel", label: "Profesyonel" }, { id: "premium", label: "Premium" }].map((t) => (
                <button key={t.id} onClick={() => setTopluTon(t.id)}
                  className={`py-2 rounded-lg border text-xs font-medium transition-all ${topluTon === t.id ? "border-rd-primary-800 bg-rd-primary-50 text-rd-primary-800" : "border-rd-neutral-200 text-rd-neutral-600 hover:border-rd-primary-800/40"}`}>
                  {t.label}
                </button>
              ))}
            </div>
          ...
          {!topluMarkaVarMi && (
            <div className="bg-rd-warning-50 border border-rd-warning-700/20 rounded-lg p-4">
              <p className="text-xs font-medium text-rd-warning-700 mb-2 flex items-center gap-1.5">
              ...
              <input type="text" value={markaOverride} onChange={(e) => setMarkaOverride(e.target.value)}
                ...
                className="w-full border border-rd-warning-700/30 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-rd-warning-700/40 bg-white" />
            </div>
          )}

          <div className="border border-rd-neutral-200 rounded-lg p-4">
            <p className="text-sm font-medium text-rd-neutral-900 mb-1">
              {topluParse.toplam} ürün işlenecek — {topluParse.toplam} kredi düşecek
            </p>
            {kullanici && !kullanici.is_admin && (
              <p className={`text-xs mb-3 ${(kullanici.kredi ?? 0) < topluParse.toplam ? "text-rd-danger-700 font-medium" : "text-rd-neutral-400"}`}>
            ...
            <button onClick={topluIslemeBaslat}
              ...
              className="w-full bg-rd-primary-800 hover:bg-rd-primary-900 disabled:bg-rd-neutral-200 text-white font-medium py-3 rounded-lg transition-colors text-sm">
              Başlat
            </button>
            <button onClick={() => { setTopluParse(null); setTopluAdim("yukle"); }}
              className="w-full mt-2 text-sm text-rd-neutral-400 hover:text-rd-neutral-600 py-2">
              Farklı dosya yükle
            </button>
          </div>
        </div>
      ...
      {(topluAdim === "islem" || topluAdim === "tamamlandi") && (
        <div className="space-y-3">
          {topluAdim === "islem" && (
            <div className="bg-rd-warning-50 border border-rd-warning-700/20 rounded-lg p-4 flex items-start gap-3">
              <AlertTriangle size={16} strokeWidth={1.5} className="text-rd-warning-700 flex-shrink-0 mt-0.5" />
              <p className="text-sm text-rd-warning-700">Üretim devam ediyor — bu sekmeyi kapatma.</p>
            </div>
          )}
          <div className="border border-rd-neutral-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-medium text-rd-neutral-900">{topluAdim === "tamamlandi" ? "Tamamlandı" : "İşleniyor..."}</p>
              <span className="text-xs text-rd-neutral-400">{topluTamamlanan} / {topluIlerlemeler.length}</span>
            </div>
            <div className="h-2 bg-rd-neutral-100 rounded-full mb-4 overflow-hidden">
              <div className="h-full bg-rd-primary-800 rounded-full transition-all duration-500"
                style={{ width: topluIlerlemeler.length ? `${(topluTamamlanan / topluIlerlemeler.length) * 100}%` : "0%" }} />
            </div>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {topluIlerlemeler.map((item) => (
                <div key={item.indeks} className="flex items-center gap-3 py-1.5 border-b border-rd-neutral-100 last:border-0">
                  <div className="w-5 flex-shrink-0 flex items-center justify-center">
                    {item.durum === "bekliyor" && <span className="w-3 h-3 rounded-full border border-rd-neutral-200 block" />}
                    {item.durum === "isleniyor" && <Loader2 size={14} strokeWidth={1.5} className="text-rd-primary-800 animate-spin" />}
                    {item.durum === "tamam" && <Check size={14} strokeWidth={2} className="text-rd-success-700" />}
                    {item.durum === "hata" && <XIcon size={14} strokeWidth={2} className="text-rd-danger-700" />}
                  </div>
                  <span className={`text-sm flex-1 truncate ${item.durum === "isleniyor" ? "text-rd-primary-800 font-medium" : item.durum === "tamam" ? "text-rd-neutral-600" : item.durum === "hata" ? "text-rd-danger-700" : "text-rd-neutral-400"}`}>
                    {item.urun}
                  </span>
                </div>
              ))}
            ...
            {topluAdim === "tamamlandi" && (
              <div className="mt-4 pt-4 border-t border-rd-neutral-100">
                <p className="text-sm text-rd-neutral-600 mb-3">
                  <span className="text-rd-success-700 font-medium">{topluTamam} başarılı</span>
                  {topluHatali > 0 && <span className="text-rd-danger-700 font-medium">, {topluHatali} hatalı</span>}
                </p>
                <button onClick={topluExcelIndir}
                  className="w-full bg-rd-success-700 hover:bg-rd-success-700 text-white font-medium py-3 rounded-lg transition-colors flex items-center justify-center gap-2 text-sm">
    ...
    <div id="giris-formu" style={{ display: aktif ? "block" : "none" }} className="mt-4 bg-white border border-rd-neutral-200 rounded-xl p-6 space-y-4">
        ...
        <div className="space-y-3">
          ...
          {!kameraAcik && !barkodBilgi && (
            <div className="bg-rd-primary-100 border border-rd-primary-800/20 rounded-lg p-5 text-center space-y-3">
              ...
              <button onClick={kameraAc} className="bg-rd-primary-800 hover:bg-rd-primary-900 text-white text-sm font-medium px-5 py-2 rounded-lg transition-colors flex items-center gap-1.5 mx-auto">
          ...
          {kameraAcik && (
            <div className="space-y-2">
              ...
              {barkodYukleniyor && <p className="text-center text-sm text-rd-neutral-600 animate-pulse">Ürün sorgulanıyor...</p>}
              <button onClick={kameraKapat} className="w-full text-xs text-rd-neutral-400 hover:text-rd-neutral-600 py-1 transition-colors">
                Kamerayı kapat
              </button>
            </div>
          )}
          {barkodBilgi && (
            <div className="bg-rd-success-50 border border-rd-success-700/20 rounded-lg p-4 space-y-1">
              <p className="text-sm font-medium text-rd-success-700">Ürün tanındı</p>
              <p className="text-sm text-rd-neutral-900"><span className="font-medium">İsim:</span> {barkodBilgi.isim}</p>
              {barkodBilgi.marka && <p className="text-sm text-rd-neutral-600"><span className="font-medium">Marka:</span> {barkodBilgi.marka}</p>}
              {barkodBilgi.kategori && <p className="text-sm text-rd-neutral-600"><span className="font-medium">Kategori:</span> {barkodBilgi.kategori}</p>}
              <button onClick={() => { setBarkodBilgi(null); setUrunAdi(""); setKategori(""); setOzellikler(""); }} className="text-xs text-rd-primary-800 hover:text-rd-primary-900 underline mt-1 transition-colors">
                Tekrar tara
              </button>
            </div>
          )}
      ...
      {(!kullanici || kullanici.anonim) && (
        <p className="text-xs text-center text-rd-neutral-400">
          Hesabın var mı?{" "}
          <button onClick={onGirisAc} className="text-rd-primary-800 hover:text-rd-primary-900 transition-colors">
            Giriş yap →
          </button>
        </p>
      )}
      ...
      {yukleniyor && (
        <div className="bg-white rounded-xl border border-rd-neutral-200 p-8 text-center space-y-4">
          <div className="flex justify-center"><div className="w-10 h-10 border-4 border-rd-neutral-200 border-t-rd-primary-800 rounded-full animate-spin" /></div>
          <p className="text-rd-neutral-600 font-medium animate-pulse">{YUKLENIYOR_MESAJLARI[yukleniyorMesaj]}</p>
          <p className="text-rd-neutral-400 text-sm">Bu birkaç saniye sürebilir...</p>
        </div>
      )}
      ...
      {sonuc && !yukleniyor && (
        <div id="sonuc-alani" className="space-y-3">
          ...
          <div className="flex justify-between items-center px-1">
            <h2 className="text-base font-medium text-rd-neutral-900">Üretilen içerik</h2>
            <button onClick={() => docxIndir(sonucBolumleri, urunAdi || "listing")} className="flex items-center gap-1.5 text-xs bg-rd-primary-100 hover:bg-rd-primary-50 text-rd-primary-800 font-medium px-3 py-1.5 rounded-lg transition-colors border border-rd-primary-800/20">
          ...
          {(() => {
            ...
            return (
              <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium ${baslikUygun ? "bg-rd-success-50 border border-rd-success-700/20 text-rd-success-700" : "bg-rd-warning-50 border border-rd-warning-700/20 text-rd-warning-700"}`}>
                {baslikUygun ? (
                  <span>✓</span>
                ) : (
          ...
          {(() => {
            ...
            return (
              <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-rd-danger-50 border border-rd-danger-700/20">
                <AlertTriangle size={14} strokeWidth={1.5} className="text-rd-danger-700 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs font-medium text-rd-danger-700">Marka/IP uyarısı</p>
                  <p className="text-xs text-rd-danger-700/80 mt-0.5">Tespit edilen marka adı: <span className="font-medium">{tekil.join(", ")}</span>. Yetkili satıcı değilseniz içeriği gözden geçirin.</p>
                </div>
              </div>
            );
          })()}
          ...
          {sonucBolumleri.map((bolum, i) => {
            ...
            return (
              <div key={i} className="bg-white rounded-xl border border-rd-neutral-200 p-5">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-sm font-medium text-rd-neutral-900">{bolum.ikon} {bolum.baslik}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-rd-neutral-200 hidden sm:block">✎ düzenlenebilir</span>
                ...
                <div ref={ref} contentEditable suppressContentEditableWarning
                  ...
                  className="whitespace-pre-wrap text-sm text-rd-neutral-600 leading-relaxed font-sans cursor-text">
                  {bolum.icerik}
                </div>
              </div>
            );
