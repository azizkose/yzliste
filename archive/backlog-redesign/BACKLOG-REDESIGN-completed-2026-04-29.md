...
## 25 — Polish-6: Tipografi sistemi + InfoStrip içerik + StepAnimation tutarlılık + video kutuları (Faz 1.97 — Aziz acceptance 6. tur)

**Aziz preview test bulguları (29 Nis P5-FIX sonrası):**
1. **InfoStrip Görsel/Video/Sosyal panelleri boş** — sadece gri placeholder kutular gözüküyor. Eski InfoStrip kodu `EXAMPLE_CONTENT.gorsel.placeholders` (string array) kullanıyor, gerçek görsel yok. `public/ornek_*.jpg` ve `public/video-ornekler/*.mp4` mevcut ama enjekte edilmemiş.
2. **Hero AppScreenshotMockup + StepAnimation video kutuları siyah** — videolar autoplay/muted/playsInline attribute'leri olmasına rağmen tarayıcıda yüklenmiyor (Aziz "siyah dikdörtgen" görüyor). StepAnimation 215. satırda ayrıca **video render etmeyen sade siyah div** var.
3. **StepAnimation aktif vs pasif kart boyutları farklı** — Step1Static kartı `px-3 py-2`, Step1Canvas (aktif) `px-4 py-2.5`. Step3Static boş `h-5` div, Step3Canvas zengin grid (h-7 image, h-7 video). → kart yüksekliği geçişlerde değişiyor, "kutu büyüklükleri aynı değil" şikayeti.
4. **Tutarsız küçük fontlar her yerde** — text-[9px], text-[10px], text-[11px] kullanımı yaygın (eyebrow, panel mini başlık, hashtag, caption). Aziz "okunamayan küçük font kalmasın" dedi.
5. **Bold kullanımı yetersiz** — Section H2 ve Hero H1'de bold yok, dikkat çekmiyor. Aziz "başlıklarda bold kullanımı uygun yerlerde uygula" dedi.

**Cowork karar (yönetiyor):**
- Önce **`docs/redesign-typography.md`** referans belgesi oluşturuldu (29 Nis Cowork). Tüm redesign sayfaları bu tabloyu uygular.
- Polish-6 tek paket — kapsamı geniş ama bir defada düzeltiliyor.
- Bu paket sonrası Aziz tek preview turu yapacak, OK ise main merge.

| ID | Başlık | Kabul Kriteri |
|---|---|---|
| P6-1 | Tipografi sistemi anasayfa kapsam | `docs/redesign-typography.md`'deki tablo Hero / StepSection / StepAnimation / InfoStrip / MarkaBilgileri'ne uygulanır. Hero H1 ve Section H2 → `font-bold` (700). Subsection H3 → `font-semibold` (600). Eyebrow → `text-xs` + `font-semibold` + tracking-[0.15em]. Tüm `text-[9px]/[10px]/[11px]` → `text-xs` (12px). |
| P6-2 | InfoStrip GorselPanel — gerçek 7 görsel | `EXAMPLE_CONTENT.gorsel.placeholders` (string array) yerine gerçek görselleri render et. 7 stil: `/ornek_beyaz.jpg`, `/ornek_dogal.jpg`, `/ornek_lifestyle.jpg`, `/ornek_ahsap.jpg`, `/ornek_mermer.jpg`, `/ornek_gradient.jpg`, `/ornek_koyu.jpg`. 4-kolon grid (mobile 2-kolon). `<img>` aspect-square + object-cover. Alt yazılarda stil adı (text-xs). Alt strip: "7 farklı stil — her stilde 1 görsel, 1 kredi" (text-xs rd-neutral-500). |
| P6-3 | InfoStrip VideoPanel — gerçek video oynat | Sol bölmede `<video src="/video-ornekler/zoom-yaklasim.mp4" muted autoPlay playsInline loop>` 9:16 portrait, w-32 md:w-40, useRef + useEffect ile playbackRate=1.5. Sağ bölmede 6 video stili list (RotateCw / ZoomIn / Lightbulb / Leaf / ScanSearch / Wind ikonları + isim, text-sm). Video tag'inde poster fallback YOK (siyah görünmemeli — autoPlay zorla başlatsın). |
| P6-4 | InfoStrip SosyalPanel — text-xs adapt | Mevcut yapı korunsun, ama tüm `text-[10px]` → `text-xs` (hashtag, platform isim). Caption `text-sm` (mevcut). EXAMPLE_CONTENT.sosyal kullanmaya devam (basketbol içeriği — Instagram + TikTok + Pinterest hazır). |
| P6-5 | InfoStrip MetinPanel + header tipografi | Mini başlık `text-[10px]` → `text-xs` + `font-semibold` + tracking-[0.1em] uppercase. Eyebrow "İÇERİK TÜRLERİ" `text-[10px]` → `text-xs` + `font-semibold`. H3 "4 içerik türü, 7 pazaryeri için" `text-xl md:text-2xl` + `font-semibold` (mevcut font-medium → semibold). Hashtag/tag pill'leri text-xs + font-medium. |
| P6-6 | Hero AppScreenshotMockup video düzelt | `components/sections/HeroBlock/AppScreenshotMockup.tsx` satır 25-32: `<video>` tag'inde `muted autoPlay playsInline loop` ZATEN var (Read'de görüldü) ama Aziz siyah görüyor → muhtemelen `bg-rd-neutral-800` background siyah, video poster yüklenirken görünüyor. **Çözüm:** `bg-rd-neutral-800` → `bg-rd-neutral-200` (açık gri fallback) + `<video>` tag'ine `preload="auto"` ekle. |
| P6-7 | StepAnimation aktif/pasif kart tutarlılığı | Step1Canvas vs Step1Static, Step2Canvas vs Step2Static, Step3Canvas vs Step3Static — **aynı padding/yapı kullansın**. Static fonksiyonları KALDIR (orphan), her step için **tek Canvas component** olsun, `inactive` prop alsın. Inactive durumda animasyon class'ları + opacity-60 efekti uygulansın, **boyut/padding değişmesin**. Step3Static'teki boş `h-5 bg-rd-neutral-100` div'i → Canvas içeriğine geç. |
| P6-8 | StepAnimation HizliVideo + 215. satır siyah div | Satır 215'teki `<div className="h-6 w-full rounded bg-rd-neutral-800">` → `<HizliVideo className="h-7 w-full rounded object-cover bg-rd-neutral-200" />` (siyah background → açık gri). HizliVideo zaten muted+autoPlay+playsInline+loop. |
| P6-9 | StepAnimation tipografi temizlik | Step başlığı (`step.title`) `text-sm font-medium` → `text-base font-semibold` (card title). Description `text-xs` (mevcut OK). Tüm `text-[9px]`, `text-[10px]` → `text-xs`. Mobile description (satır 510) `text-xs` mevcut OK. |
| P6-10 | MarkaBilgileri + diğer anasayfa elemanları | components/sections/MarkaBilgileriSection.tsx + LandingFAQ + AnasayfaCta vb. → eyebrow text-xs + font-semibold, H2 font-bold. text-[9-11px] varsa text-xs. |

#### P6-1~P6-10 Birleşik Prompt

```
ÖNEMLİ — KURAL OVERRIDE (BÜYÜK HARFLE — DİKKAT):
Bu görev `claude/redesign-modern-ui` branch'inde. CLAUDE.md UI 
kuralları GEÇERSİZ. ÖNCELİKLE OKU: docs/redesign-typography.md
referans tablosunu açıp bak. Bu tabloya uy.

CLAUDE.md "font-bold yasak" diyor olabilir → BU BRANCH'TE GEÇERLİ DEĞİL.
Hero H1 ve Section H2'de font-bold ZORUNLU.
text-[9px]/[10px]/[11px] HER YERDE text-xs ile değiştir.

Branch: claude/redesign-modern-ui (devam et, yeni branch açma)
Görev: P6-1~P6-10 — Polish-6 (Cowork yönetiminde, 10 madde)

ARKAPLAN:
Aziz P5-FIX sonrası preview'da gördü: 
1. InfoStrip Görsel/Video/Sosyal panelleri gri placeholder
2. Hero ve StepAnimation video kutuları siyah görünüyor
3. StepAnimation aktif/pasif kart boyutları farklı
4. Sayfada text-[9px]/[10px] yaygın, okunamıyor
5. Section başlıkları yeterince dikkat çekmiyor (bold yok)

GÖREVLER (sırayla, ayrı commit'ler):

═══════════════════════════════════════════════════════════════════
P6-1: docs/redesign-typography.md OKU + anasayfa global tipografi
═══════════════════════════════════════════════════════════════════

ÖNCELİKLE: docs/redesign-typography.md dosyasını Read tool ile aç,
tabloyu içselleştir.

Sonra bu dosyalarda eyebrow / H2 / H3 tipografi düzelt:
- components/sections/HeroBlock/HeroSection.tsx
- components/landing/StepSection.tsx (h2 font-medium → font-bold + text-3xl/4xl)
- components/sections/MarkaBilgileriSection.tsx (h2 + eyebrow)
- components/landing/LandingFAQ.tsx
- components/landing/AnasayfaCta.tsx (varsa)
- app/_tanitim-redesign.tsx (kullanılıyorsa)

Kural:
- Section H2: text-3xl md:text-4xl + font-bold + Manrope display
- Eyebrow: text-xs + font-semibold + tracking-[0.15em] uppercase + rd-warm-700
- text-[9-11px] → text-xs HER YERDE

Commit: feat(redesign): P6-1 anasayfa tipografi sistemi (font-bold H2 + text-xs eyebrow)

═══════════════════════════════════════════════════════════════════
P6-2: InfoStrip GorselPanel — 7 gerçek görsel
═══════════════════════════════════════════════════════════════════

Dosya: components/landing/InfoStrip.tsx, GorselPanel fonksiyonu (satır 96)

Mevcut: EXAMPLE_CONTENT.gorsel.placeholders (string array) → grid + ikon + label
Yeni: 7 gerçek basketbol topu görseli, grid render

Yapı (referans):

  const ORNEK_GORSELLER = [
    { src: '/ornek_beyaz.jpg', label: 'Beyaz zemin' },
    { src: '/ornek_dogal.jpg', label: 'Doğal ortam' },
    { src: '/ornek_lifestyle.jpg', label: 'Lifestyle' },
    { src: '/ornek_ahsap.jpg', label: 'Ahşap zemin' },
    { src: '/ornek_mermer.jpg', label: 'Mermer zemin' },
    { src: '/ornek_gradient.jpg', label: 'Gradient' },
    { src: '/ornek_koyu.jpg', label: 'Koyu zemin' },
  ]
    ...
    return (
      <div>
        <div className="grid grid-cols-3 md:grid-cols-4 gap-2 md:gap-3">
          {ORNEK_GORSELLER.map(({src, label}) => (
            <div key={src} className="flex flex-col gap-1.5">
              <div className="aspect-square w-full rounded-lg overflow-hidden bg-rd-neutral-100">
                <img src={src} alt={`Basketbol topu ${label}`} 
                     className="w-full h-full object-cover" 
                     loading="lazy" />
              </div>
              <span className="text-xs text-rd-neutral-500 text-center leading-tight">{label}</span>
            </div>
          ))}
        </div>
        <p className="mt-3 text-xs text-rd-neutral-500">7 farklı stil — her stilde 1 görsel, 1 kredi.</p>
      </div>
    )
  }

next/image kullanma — basit <img> tag yeterli (next/image SSR sorunu olabilir).

Commit: feat(landing): P6-2 InfoStrip GorselPanel gerçek 7 görsel

═══════════════════════════════════════════════════════════════════
P6-3: InfoStrip VideoPanel — gerçek video + 6 stil listesi
═══════════════════════════════════════════════════════════════════

Dosya: components/landing/InfoStrip.tsx, VideoPanel fonksiyonu (satır 115)

Mevcut: EXAMPLE_CONTENT.video metadata + sade placeholder div
Yeni: Sol gerçek video + sağ 6 stil listesi

Yapı:

  import { RotateCw, ZoomIn, Lightbulb, Leaf, ScanSearch, Wind } from 'lucide-react'

  const VIDEO_STILLERI = [
    { Ikon: RotateCw, baslik: '360° Dönüş', aciklama: 'Tüm açılar görünür' },
    { Ikon: ZoomIn, baslik: 'Zoom yaklaşım', aciklama: 'Doku ve detay' },
    { Ikon: Lightbulb, baslik: 'Dramatik ışık', aciklama: 'Premium his' },
    { Ikon: Leaf, baslik: 'Doğal ortam', aciklama: 'Organik sıcak' },
    { Ikon: ScanSearch, baslik: 'Detay tarama', aciklama: 'İşçilik vurgu' },
    { Ikon: Wind, baslik: 'Kumaş hareketi', aciklama: 'Akışkanlık' },
  ]
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start">
        <div className="w-32 md:w-40 shrink-0">
          <video
            ...
            className="w-full rounded-lg bg-rd-neutral-200"
            style={{ aspectRatio: '9/16', objectFit: 'cover' }}
            aria-hidden="true"
          />
        </div>
        <div className="flex-1">
          <p className="text-xs font-semibold tracking-[0.1em] uppercase text-rd-neutral-500 mb-2">6 video stili</p>
          <div className="grid grid-cols-2 gap-2">
            {VIDEO_STILLERI.map(({Ikon, baslik, aciklama}) => (
              <div key={baslik} className="flex items-start gap-2 p-2 rounded-lg bg-rd-neutral-50">
                <Ikon size={16} strokeWidth={1.5} className="text-rd-primary-700 shrink-0 mt-0.5" />
                <div className="min-w-0">
                  <p className="text-xs font-medium text-rd-neutral-900 truncate">{baslik}</p>
                  <p className="text-xs text-rd-neutral-500 truncate">{aciklama}</p>
                </div>
              </div>
            ))}
          </div>
          <p className="mt-3 text-xs text-rd-neutral-500">5 saniye 1 kredi · 10 saniye 2 kredi · Stil seçilir, AI üretir.</p>
        </div>
      </div>
    )
  }

useState/useEffect/useRef import'larında useRef varsa OK, yoksa ekle.

Commit: feat(landing): P6-3 InfoStrip VideoPanel gerçek video + 6 stil

═══════════════════════════════════════════════════════════════════
P6-4: InfoStrip SosyalPanel — text-xs adapt
═══════════════════════════════════════════════════════════════════

Dosya: components/landing/InfoStrip.tsx, SosyalPanel fonksiyonu (satır 146)

Mevcut yapı korunsun (EXAMPLE_CONTENT.sosyal kullanıyor — basketbol içeriği hazır).
Sadece tipografi:
  text-[10px] → text-xs (hashtag, platform header)
  caption text-sm korunsun
  hashtag font-medium ekle

Commit: feat(landing): P6-4 InfoStrip SosyalPanel text-xs

═══════════════════════════════════════════════════════════════════
P6-5: InfoStrip MetinPanel + header tipografi
═══════════════════════════════════════════════════════════════════

Dosya: components/landing/InfoStrip.tsx

Header (satır 207-218):
  Eyebrow text-[10px] → text-xs + font-semibold (mevcut font-medium → semibold)
  H3 font-medium → font-semibold (P5'te yanlışlıkla medium yapılmıştı)

MetinPanel (satır 54):
  Mini başlık text-[10px] → text-xs + font-semibold + tracking-[0.1em] uppercase
  Title/description text-sm korunsun
  Tag pill text-xs + font-medium (mevcut)

Tab kartları (satır 246-271):
  Description text-xs (mevcut OK)
  kredi pill text-[10px] → text-xs + font-medium
  süre text-[10px] → text-xs

Commit: feat(landing): P6-5 InfoStrip header + MetinPanel + tab tipografi

═══════════════════════════════════════════════════════════════════
P6-6: Hero AppScreenshotMockup siyah background fix
═══════════════════════════════════════════════════════════════════

Dosya: components/sections/HeroBlock/AppScreenshotMockup.tsx

Satır 32 (yaklaşık):
  className="h-8 w-full rounded object-cover bg-rd-neutral-800"
  → 
  className="h-8 w-full rounded object-cover bg-rd-neutral-200"

video tag'ine preload="auto" ekle (yoksa).

Commit: fix(hero): P6-6 video kutusu açık gri fallback (siyah → neutral-200)

═══════════════════════════════════════════════════════════════════
P6-7: StepAnimation aktif/pasif kart tutarlılığı (BÜYÜK refactor)
═══════════════════════════════════════════════════════════════════

Dosya: components/landing/StepAnimation.tsx

SORUN: Step1Static/Step2Static/Step3Static fonksiyonları aktif Canvas'tan
farklı padding/içerik kullanıyor. Kart yüksekliği geçişlerde değişiyor.

ÇÖZÜM: 3 ayrı Static fonksiyonu KALDIR. Her step için tek Canvas component
olsun, `inactive` prop alsın. Inactive durumda:
- Animasyon class'ları yok
- opacity-60 (içerik görünür ama soluk)
- Padding/yapı/boyut AYNI
  ...
  function Step1Canvas({ reduced, inactive }: { reduced: boolean; inactive?: boolean }) {
    return (
      <div aria-hidden="true" className={cn(
        "flex flex-col items-center justify-center gap-3 px-4 py-4",
        inactive && "opacity-60"
      )}>
        <div className={cn('flex items-center gap-3', !reduced && !inactive && 'animate-step-photo-in')}>
          {/* Aynı kutular px-4 py-2.5 — Static'teki px-3 py-2 KULLANMA */}
          ...
        </div>
        <div className={cn('w-full max-w-[220px]', !reduced && !inactive && 'animate-step-text-in')}>
          {/* Aynı içerik — Static'teki "Basketbol topu..." değil, aktif Canvas'taki tam metin */}
          ...
        </div>
      </div>
    )
...
Step2Canvas, Step3Canvas için de aynı pattern.

Step3Canvas kart içeriği: video/image/text hepsi h-7 — inactive durumda da
aynı h-7 + opacity-60. Boş div (h-5 bg-rd-neutral-100) KULLANMA.

STEPS array'inde StaticCanvas alanını kaldır:
  STEPS = [
    { number: '1', title: ..., description: ..., Canvas: Step1Canvas },
    ...
  ]

Render:
  {isActive ? (
    <Canvas reduced={reduced} />
  ) : (
    <Canvas reduced={true} inactive />
  )}

Commit: refactor(landing): P6-7 StepAnimation tek Canvas pattern (aktif/pasif tutarlı)

═══════════════════════════════════════════════════════════════════
P6-8: StepAnimation siyah div → gerçek video
═══════════════════════════════════════════════════════════════════

Dosya: components/landing/StepAnimation.tsx, satır 215
...
Yeni — bu fallback hiç render edilmemeli (P6-7'den sonra). Ama emniyet için:
  <div className="h-7 w-full rounded bg-rd-neutral-200" />

(Açık gri fallback, siyah değil.)

Ayrıca: HizliVideo bg-rd-neutral-800 → bg-rd-neutral-200 (satır 32 ve 210).

Commit: fix(landing): P6-8 StepAnimation siyah fallback → açık gri

═══════════════════════════════════════════════════════════════════
P6-9: StepAnimation tipografi temizlik
═══════════════════════════════════════════════════════════════════

Dosya: components/landing/StepAnimation.tsx

Tüm text-[9px] → text-xs (10+ yerde):
  - VEYA etiketi (satır 51, 253)
  - Step3 "AI üretiyor..." (satır 189, 309)
  - Step3 kart label (satır 205, 318)
  - Step3 kart text (satır 208)
  - Step3 İndir button (satır 227)

Tüm text-[10px] → text-xs:
  - Pazaryeri/içerik chip (satır 107, 129, 277, 291)
  - Step1 Fotoğraf/Açıklama label (satır 251, 256)
  - Step1 typing preview (satır 67, 260)
  - Step2 mini başlık (satır 99, 121)

Step başlığı (satır 452): text-sm font-medium → text-base font-semibold
Step description (satır 459): text-xs (mevcut OK)

Commit: feat(landing): P6-9 StepAnimation tipografi temizlik (text-xs min)

═══════════════════════════════════════════════════════════════════
P6-10: MarkaBilgileri + LandingFAQ + AnasayfaCta tipografi
═══════════════════════════════════════════════════════════════════

components/sections/MarkaBilgileriSection.tsx:
  Eyebrow text-[9-10px] → text-xs + font-semibold + tracking-[0.15em]
  H2 font-medium/semibold → font-bold + text-3xl/4xl
  text-[9-11px] varsa text-xs

components/landing/LandingFAQ.tsx (varsa):
  H2 font-bold + text-3xl/4xl
  Soru başlığı text-base + font-semibold
  Cevap text-sm
...
Diğer anasayfa importları (app/_tanitim-redesign.tsx ve app/page.tsx):
  Bölüm başlıkları font-bold + text-3xl/4xl

Commit: feat(redesign): P6-10 MarkaBilgileri + diğer anasayfa H2 font-bold

═══════════════════════════════════════════════════════════════════

ACCEPTANCE (Code kendi yapsın bitiş öncesi):
[ ] docs/redesign-typography.md okundu, tablo uygulandı
[ ] Hiçbir text-[9px], text-[10px], text-[11px] kalmadı (grep ile kontrol)
[ ] Hero H1 + Section H2 → font-bold (700)
[ ] H3 (InfoStrip) → font-semibold (600)
[ ] Eyebrow → text-xs + font-semibold + tracking-[0.15em]
[ ] InfoStrip GorselPanel 7 gerçek görsel render ediyor
[ ] InfoStrip VideoPanel gerçek video oynuyor + 6 stil sağda listeli
[ ] InfoStrip SosyalPanel basketbol caption + hashtag
[ ] Hero AppScreenshotMockup video + StepAnimation video açık gri (siyah değil)
[ ] StepAnimation Static fonksiyonları silindi, tek Canvas pattern
[ ] StepAnimation aktif/pasif kart yüksekliği aynı (manuel test)
[ ] npm run build temiz
[ ] npm run lint temiz

Rapor (bitince):
- Hangi dosyalar değişti (özet)
- Commit hash listesi (10 commit)
- npm run build çıktısı
- text-[9-11px] grep sonucu (boş olmalı)
- Açık riskler (P6-7 büyük refactor — animasyon class'larında bozulma var mı?)
```
...
## 26 — Polish-7: InfoStrip → canlı FeaturesTabbed pattern transplant (Faz 1.98 — Aziz acceptance 7. tur)

**Aziz preview test bulguları (29 Nis P6 sonrası):**
1. **Video paneli sol tarafta video oynamıyor** — statik basketbol topu görseli görünüyor (ekran görüntüsü). Code "1 büyük video + sağda 6 isim listesi" yapısı yetmedi.
2. **Metin paneli "kopyala" butonsuz** — canlı FeaturesTabbed'da her bölümde (Başlık/Özellikler/Açıklama/Etiketler) sağ üstte Kopyala rozet/buton var. InfoStrip'te yok.
3. **Açıklayıcı değil** — canlı sitede her panelde intro paragraph + sub-başlık + bottom CTA + zengin yapı var. InfoStrip panelleri sade.

**Cowork yönetim kararı (Aziz onaylı):**
- InfoStrip'in **4 üst kart + accordion** yapısı korunsun (Aziz net "1a" dedi)
- "Detaya bak" toggle butonu **üstten alta** taşı — accordion'un açılacağı yere yakın olsun, anlamlı
- Panellerin **içeriği canlı `components/tanitim/FeaturesTabbed.tsx`'tan birebir port** edilsin — intro paragraph, 3 platform tab (Metin), kopyala butonları, bottom CTA, 6 ayrı video kartı, vb.
- 6 video → her biri kendi `<video autoPlay loop muted playsInline>` ile aynı anda oynar (canlı pattern)
- CTA "Listing metni üret →" / "Stüdyo görseli üret →" / "Ürün videosu üret →" / "Sosyal içerik üret →" — her panel altına, /uret?tab=... route

**Önemli not Code için:** Bu **büyük transplant**. components/tanitim/FeaturesTabbed.tsx tam canlı pattern, oradan **birebir kopyala** + sadece hardcoded hex → rd-* token swap. Yeni-yorum tasarım YASAK.

| ID | Başlık | Kabul Kriteri |
|---|---|---|
| P7-1 | "Detaya bak" toggle alta taşı | InfoStrip header (eyebrow + h3 + subtitle) **sağ tarafa toggle KOYMA**. 4 kart grid'in **altında**, ortalı (`flex justify-center mt-4`), `Detaya bak ▼` butonu olsun. Aynı state (`detailOpen`) korunsun, animasyon (chevron rotation) çalışsın. |
| P7-2 | MetinPanel canlı pattern transplant | Mevcut sade MetinPanel kaldır. Yerine canlı FeaturesTabbed'in `ozellikTab === 0` bloğu (satır 189-247). Yapı: intro paragraph (text-sm rd-neutral-600) + 3 platform pill tab (Trendyol orange/Amazon E47911/Etsy rose) + aciklamaKisa (text-xs rd-neutral-500) + 4 bölüm kartı (BolumIkon + başlık + sağda **KopyalaButon** + whitespace-pre-line metin + border-l renk) + bottom feature check (4 madde) + mavi info kutu ("Her pazaryerinin kuralları farklı...") + CTA `/uret?tab=metin`. EXAMPLE_CONTENT_TR.metin'den çek (basketbol Trendyol/Amazon, bakır cezve Etsy). |
| P7-3 | GorselPanel canlı pattern transplant | Mevcut 7 görsel grid yerine canlı `ozellikTab === 1` bloğu (satır 249-311). Yapı: intro paragraph + sub-başlık "Tek fotoğraftan 7 farklı stüdyo stili" + sub-açıklama "Stil başına 1 kredi · Üretimde düşer..." + **2 satırlı grid**: 1. satır 4-kolon (ham fotoğraf özel kart "Dağınık arka plan" rd-danger rozet + 3 stil: Beyaz, Koyu, Lifestyle) + 2. satır 4-kolon (Mermer, Ahşap, Gradient, Doğal) + "3 farklı yöntemle sahne oluştur" + 3 chip (Hazır stiller, Kendi promptun, Arka plan fotoğrafı) + CTA `/uret?tab=gorsel`. |
| P7-4 | VideoPanel canlı pattern transplant — **KRİTİK** | Mevcut "1 video + sağda liste" YANLIŞ. Yerine canlı `ozellikTab === 2` bloğu (satır 314-371). Yapı: intro paragraph + sub-başlık "Ürün fotoğrafından tanıtım videosu" + sub-açıklama + **2-kolon grid 6 video kartı** — her kartta sol `<video src="/video-ornekler/{stil}.mp4" autoPlay loop muted playsInline className="w-24 h-24 rounded-lg object-cover">` + sağda ikon + başlık + açıklama (text-xs rd-neutral-500) + **3 özellik kartı** (5sn 10kr, 10sn 20kr, 3 format) + "Nasıl çalışır?" 4-step + CTA `/uret?tab=video`. **6 video aynı anda oynamalı** — her birinin kendi tag'i. playbackRate ayarlama gerekmiyor. |
| P7-5 | SosyalPanel canlı pattern transplant | Mevcut Instagram/TikTok/Pinterest yapısı yerine canlı `ozellikTab === 3` bloğu (satır 373-425). Yapı: intro paragraph + sub-başlık + 4 platform pill (Instagram/TikTok/Facebook/Twitter — yeşil rd-success-50/700) + 3 ayrı kart: Instagram Caption (Camera ikonu + Kopyala) + Hashtag Seti (Tag ikonu + Kopyala, rd-success metin) + Twitter/X 280 karakter (Kopyala) + CTA `/uret?tab=sosyal`. EXAMPLE_CONTENT.sosyal'dan basketbol içeriği çekilebilir veya canlı pattern'deki Selin Porselen örnekleri yerine basketbol metni inline yazılabilir. |
| P7-6 | KopyalaButon inline component (reusable) | InfoStrip içinde basit inline KopyalaButon function: `useState` + `navigator.clipboard.writeText` + 1.8s sonra "Kopyalandı" → "Kopyala". ClassName: `text-xs bg-white border border-rd-neutral-200 text-rd-neutral-500 hover:text-rd-neutral-800 px-2 py-0.5 rounded-lg transition-colors`. metin prop alır. Kopyalandığında `text-rd-success-700` + Check ikonu. Tüm Kopyala rozetleri bu component ile değiştir. |
| P7-7 | InfoStrip header sağ üst toggle KALDIR | Header'daki `<button>Detaya bak ▼</button>` (sağ üstte) tamamen kaldırılsın. Header sadece eyebrow + h3 + subtitle. P7-1'le birlikte alta yeni buton zaten geldi. |

#### P7-1~P7-7 Birleşik Prompt

```
ÖNEMLİ — KURAL OVERRIDE (BÜYÜK HARFLE):
Bu görev `claude/redesign-modern-ui` branch'inde. CLAUDE.md UI 
kuralları GEÇERSİZ. docs/redesign-typography.md tablosuna uy.

font-bold (Hero H1, Section H2) ZORUNLU bu branch'te.
text-[9-11px] yasak, text-xs (12px) min.

Branch: claude/redesign-modern-ui (devam et)
Görev: P7-1~P7-7 — Polish-7 InfoStrip → canlı FeaturesTabbed pattern

ARKAPLAN:
P6 sonrası Aziz preview test:
- VideoPanel sol video oynamıyor (statik basketbol görseli)
- MetinPanel kopyala butonu yok
- "Açıklayıcı değil" — canlı zenginlik yok

Cowork yönetim kararı (Aziz onaylı):
- InfoStrip 4 kart + accordion yapısı korunsun
- "Detaya bak" toggle alta taşınsın
- 4 panel İÇERİĞİ canlı `components/tanitim/FeaturesTabbed.tsx`'tan
  BİREBİR PORT edilsin (intro + Kopyala + CTA + zengin yapı)
- 6 video → her biri kendi tag'iyle aynı anda oynar
- Bottom CTA her panele

REFERANS DOSYA (oku):
components/tanitim/FeaturesTabbed.tsx — canlı pattern, 431 satır.
Hardcoded hex var (#1E4DD8, #5A5852 vb) — rd-* token'a çevir:
  #1E4DD8 → rd-primary-700
  #163B9E → rd-primary-800  
  #BAC9EB → rd-primary-200
  #F0F4FB → rd-primary-50
  #1A1A17 → rd-neutral-900
  #5A5852 → rd-neutral-600
  #908E86 → rd-neutral-400
  #D8D6CE → rd-neutral-200
  #F1F0EB → rd-neutral-100
  #FAFAF8 → rd-neutral-50
  #0F5132 → rd-success-700
  #E8F5EE → rd-success-50
  #FCECEC → rd-danger-50
  #7A1E1E → rd-danger-700
  #E47911 → bırak hardcoded (Amazon brand color)
  orange-400/orange-500 → bırak (Trendyol)
  rose-400/rose-500 → bırak (Etsy)

GÖREVLER (sırayla, ayrı commit'ler):

═══════════════════════════════════════════════════════════════════
P7-1: "Detaya bak" toggle alta taşı
═══════════════════════════════════════════════════════════════════

Dosya: components/landing/InfoStrip.tsx

Header bloğu (satır ~206-238):
  Eski: <div className="flex items-start justify-between mb-5">
    <div>eyebrow + h3 + subtitle</div>
    <button>Detaya bak ▼</button>
  </div>
...
Sonra 4 kart grid (mevcut kalır).

4 kart grid'in HEMEN ALTINA yeni buton:
  ...
  {detailOpen && (
    detay alanı — mevcut yapı korunur, ama içerik P7-2~P7-5'te zenginleşir
  )}

P7-7 (header sağ üst toggle kaldır) bu commit içinde dahildir.

Commit: refactor(landing): P7-1+7 InfoStrip detay toggle alta taşındı

═══════════════════════════════════════════════════════════════════
P7-6: KopyalaButon inline component
═══════════════════════════════════════════════════════════════════

InfoStrip.tsx içinde, panel function'larından önce:
...
Lucide Check import'a ekle.

Commit: feat(landing): P7-6 InfoStrip KopyalaButon inline component

═══════════════════════════════════════════════════════════════════
P7-2: MetinPanel canlı pattern transplant
═══════════════════════════════════════════════════════════════════

InfoStrip.tsx — MetinPanel function'ını TAMAMEN değiştir.

Referans: canlı FeaturesTabbed satır 189-247 (ozellikTab === 0 bloğu).

Yeni yapı:

1. Önce platform verileri ve BolumIkon helper'ını InfoStrip dosyasına ekle
   (canlı FeaturesTabbed satır 7-130'dan port — ama EXAMPLE_CONTENT_TR.metin
   kullansın, hardcoded Selin Porselen YERİNE).
   ...
   const platformVerileriInfoStrip: Record<MarketId, {
     ...
     amazon: { /* aynı yapı, EXAMPLE_CONTENT_TR.metin.amazon */ etiket: "Amazon TR", ... },
     etsy: { /* aynı yapı, EXAMPLE_CONTENT_TR.metin.etsy */ etiket: "Etsy", ... },
   }
      ...
      <div>
        ...
        <div className="flex gap-2 mb-1">
          {(Object.keys(platformVerileriInfoStrip) as MarketId[]).map((key) => {
            const p = platformVerileriInfoStrip[key]
            const aktif = market === key
            const renkMap: Record<MarketId, string> = {
              trendyol: aktif ? "bg-orange-500 text-white" : "bg-rd-neutral-100 text-rd-neutral-600 hover:bg-rd-neutral-200",
              amazon:   aktif ? "bg-[#E47911] text-white"  : "bg-rd-neutral-100 text-rd-neutral-600 hover:bg-rd-neutral-200",
              etsy:     aktif ? "bg-rose-500 text-white"   : "bg-rd-neutral-100 text-rd-neutral-600 hover:bg-rd-neutral-200",
            }
            return (
              <button
                key={key}
...
InfoStrip ana component'inde MetinPanel render'ı: MetinPanel'a market + setMarket prop ver.
  {activeTab === 'metin' && <MetinPanel market={activeMarket} setMarket={setActiveMarket} />}

Üstteki ayrı MARKETS chip alanını (canlı'da yok) KALDIR — platform tab'ı MetinPanel içine geldi.

Commit: feat(landing): P7-2 MetinPanel canlı pattern + Kopyala + CTA

═══════════════════════════════════════════════════════════════════
P7-3: GorselPanel canlı pattern transplant
═══════════════════════════════════════════════════════════════════

InfoStrip.tsx — GorselPanel function'ını TAMAMEN değiştir.

Referans: canlı FeaturesTabbed satır 249-311.
...
X ikonu zaten import edilmiş olmalı, yoksa lucide-react'tan ekle.

Commit: feat(landing): P7-3 GorselPanel canlı pattern + 7 stil + CTA

═══════════════════════════════════════════════════════════════════
P7-4: VideoPanel canlı pattern transplant — KRİTİK
═══════════════════════════════════════════════════════════════════

InfoStrip.tsx — VideoPanel function'ını TAMAMEN değiştir.

Referans: canlı FeaturesTabbed satır 314-371.
  ...
  function VideoPanel() {
    return (
      <div>
        ...
        {/* 6 video kartı 2-kolon — HER BİR KART KENDİ VIDEO TAG'İYLE */}
...
KRİTİK: 6 video tag'i AYRI AYRI render edilir, her biri autoPlay loop muted
playsInline preload="auto" attribute'leriyle. ÖNEMLİ: w-24 h-24 sabit boyut.
useRef veya playbackRate gerekmiyor — canlı pattern'de yok, default hızda.

Tüm Lucide importları: RotateCw, ZoomIn, Lightbulb, Leaf, ScanSearch, Wind,
Timer, Film, Columns2 — yoksa ekle.

Commit: feat(landing): P7-4 VideoPanel 6 ayrı video kartı canlı pattern

═══════════════════════════════════════════════════════════════════
P7-5: SosyalPanel canlı pattern transplant
═══════════════════════════════════════════════════════════════════

InfoStrip.tsx — SosyalPanel function'ını TAMAMEN değiştir.

Referans: canlı FeaturesTabbed satır 373-425.
  ...
  function SosyalPanel() {
    ...
    return (
      <div>
        ...
        {/* 3 kart: Instagram Caption, Hashtag, Twitter */}
        <div className="space-y-3">
          {/* Instagram Caption */}
...
Camera, Tag Lucide import'larına ekle.

Commit: feat(landing): P7-5 SosyalPanel canlı pattern + Kopyala + CTA

═══════════════════════════════════════════════════════════════════

ACCEPTANCE (Code kendi yapsın bitiş öncesi):
[ ] InfoStrip header'da sağ üst toggle YOK, alta taşınmış (4 kart altında ortalı)
[ ] KopyalaButon component navigator.clipboard ile çalışıyor + 1.8s feedback
[ ] MetinPanel: intro + 3 platform tab (Trendyol/Amazon/Etsy) + 4 bölüm kartı (her biri Kopyala) + bottom feature check + mavi info + CTA
[ ] GorselPanel: intro + 7 stil grid (1 ham + 7) + 3 yöntem chip + CTA
[ ] VideoPanel: 6 AYRI video tag, hepsi aynı anda oynar (autoPlay loop muted playsInline) + 3 özellik kartı + Nasıl çalışır + CTA
[ ] SosyalPanel: 4 platform pill + 3 kart (Caption/Hashtag/Twitter) hepsi Kopyala + CTA
[ ] Tüm CTA'lar /uret?tab=metin/gorsel/video/sosyal route'larına gidiyor
[ ] Hardcoded hex sadece Trendyol orange-500, Amazon #E47911, Etsy rose-500 (brand colors), gerisi rd-* token
[ ] text-[9-11px] yok, text-xs (12px) min
[ ] Lucide import'ları tam (RotateCw, ZoomIn, Lightbulb, Leaf, ScanSearch, Wind, Timer, Film, Columns2, Camera, Tag, Hash, FileText, Check, X, ChevronDown)
[ ] npm run build temiz
[ ] Yeni-yorum tasarım YOK — canlı FeaturesTabbed.tsx birebir port

Rapor:
- 7 commit hash
- npm run build çıktısı (özet)
- Hangi panellerde Kopyala kaç adet
- 6 video tag'i preview'da hepsi oynuyor mu (Code'un browser test gerekirse manuel)
- Açık riskler (browser autoplay policy)
```
...
## 27 — Polish-8: Logo + /uret cost + tipografi tutarlılık + ton metinleri + InfoStrip görsel hiyerarşi (Faz 1.99 — Aziz acceptance 8. tur)

**Kapsam:** 5 ana madde, tek pakette. Aziz onaylı plan (29 Nis):
- Logo: "yzliste Beta" → "yzliste" (Beta badge kalksın)
- /uret StickySubmitBar: cost=1 default sorunu (henüz seçim yokken bile 1 kredi diyor)
- Anasayfa tipografi tutarsızlığı: SectionHeader'da extrabold/text-5xl, Hero'da slate-* token, arbitrary text-[13/15px]
- MarkaBilgileri 3 ton metni: samimi "siz" hitabı, profesyonel "tin kaplama" (terim hatası), premium markaDolu ile örtüşme
- InfoStrip detay alanı: bg-rd-neutral-50 + iç kartlar aynı renk → "alanlar belli değil" şikayeti

| ID | Başlık | Kabul Kriteri |
|---|---|---|
| P8-1 | Logo "yzliste Beta" → "yzliste" (Beta kaldır) | components/SiteHeader.tsx — eğer "Beta" badge varsa kaldır. lib/constants/hero.ts NAV_BRAND.betaBadge true → false. components/sections/HeroBlock/Nav.tsx içinde "Beta" badge render eden kısım kaldırılsın. Logo image (/yzliste_logo.png) korunsun. **NOT:** Eğer /yzliste_logo.png içinde "Beta" yazıyorsa Aziz yeni logo dosyası yükleyene kadar img bırak — sadece kod tarafındaki "Beta" textli badge kaldırılsın. |
| P8-2 | /uret StickySubmitBar — cost=0 durumu | `components/uret/useCalculateCredits.ts`: gorsel için `Math.max(1, selectedStylesCount)` → `selectedStylesCount` (0 dönsün), sosyal için `Math.max(1, selectedPlatformsCount)` → `selectedPlatformsCount`. Metin için: parametreye `productName?: string`, `hasPhoto?: boolean` ekle, `productName.trim() === '' && !hasPhoto` ise return 0. Video için: `hasPhoto: boolean` ekle, `!hasPhoto` ise return 0 (yani fotoğraf yok ise üretim olmaz). app/uret/page.tsx satır 137'de calculateCredits çağrısına bu prop'ları gönder. **StickySubmitBar:** `cost === 0` ise sol tarafı: `<span className="text-2xl font-bold text-rd-neutral-300 tabular-nums">—</span>` + alt yazı "Önce form alanlarını doldurun" (rd-neutral-500). cost > 0 olduğunda mevcut "{cost} kredi" + "Üretim sonrası: X kredi kalır" yapısı. |
| P8-3 | Anasayfa tipografi tutarlılık (5 alt madde) | (a) `components/primitives/SectionHeader.tsx` h2: `font-extrabold + text-3xl md:text-4xl lg:text-5xl` → `font-bold + text-3xl md:text-4xl` (lg:text-5xl kaldır). (b) `components/sections/HeroBlock/HeroContent.tsx` h1: `text-slate-900` → `text-rd-neutral-900`, `font-[800]` → `font-bold`. (c) HeroContent subtitle ve diğer text-slate-* → text-rd-neutral-*. (d) Arbitrary `text-[13px]`, `text-[15px]` → `text-sm` veya `text-base`. (e) `components/sections/HeroBlock/TrustStrip.tsx` `border-slate-200` → `border-rd-neutral-200`. (f) Hero alt + StepSection üst arası boşluk: HeroSection'dan StepSection.tsx'in üst pt'sini incele — eğer toplam çok büyükse StepSection üst pt-12 (alt aynı kalır). |
| P8-4 | MarkaBilgileri 3 ton metni yenile | `lib/constants/marka-bilgileri.ts` TONE_CHIPS array'i 3 metin yenilensin: **samimi** = "Sabah kahvene başlamadan bir bakar mısın? Bu cezve seninle kalmak için yapılmış. Gaziantep'te elle dövülmüş, kalay kaplı içi, hediye kutusunda — sevdiğine vermek için de tam zamanı." (sen hitabı, günlük dil). **profesyonel** = "El dövme bakır cezve; gıda güvenli kalay kaplı iç yüzey, 250 ml kapasite, 2 porselen fincan ile komple set. Gaziantep'te aile atölyesinden, her parça ayrı kalite kontrolünden geçer." ("tin" → "kalay" terim düzeltmesi). **premium** = "Bakırın sıcaklığı, sabah kahvesini bir ritüele dönüştürür. El dövme yüzeyin her dokunuşunda Gaziantep ustalarının izi var. Hediye kutusunda, kullanım rehberi ile birlikte — özel anlar için biçilmiş bir hediye." (markaDolu metniyle örtüşme yok, marka adı kullanılmadı). **+ UX clarification:** MarkaBilgileriSection sol kolonda "Metin tonu seçimi" ile "Marka bilgisi (boş/dolu)" arasındaki farkı netleştir — `<p>` ile küçük açıklama: "**Metin tonu** kelime tercihini belirler · **Marka bilgisi** içeriğe ne kadar kişisel detay katar" (text-xs text-rd-neutral-500). |
| P8-5 | InfoStrip görsel hiyerarşi (KRİTİK) | `components/landing/InfoStrip.tsx` detay alanı (satır ~502): `className="mt-4 rounded-xl border border-rd-neutral-200 bg-rd-neutral-50 p-4 md:p-5"` → `className="mt-4 rounded-xl border border-rd-neutral-200 bg-rd-neutral-100 p-5 md:p-7"`. **İç kart background swap:** Tüm panellerin içindeki kartlar (`bg-rd-neutral-50` olanlar) → `bg-white`. Hangi kartlar etkilenir: MetinPanel bölüm kartları (satır 150) `bg-rd-neutral-50` → `bg-white`, GorselPanel görsel kartları (219, 236) korunur (zaten beyaz). VideoPanel 6 video kartı (281) `bg-rd-neutral-50` → `bg-white`. VideoPanel 3 özellik kartı (308) `bg-rd-neutral-50` → `bg-white`. VideoPanel "Nasıl çalışır" (317) `bg-rd-neutral-100` → `bg-white`. SosyalPanel 3 caption kartı (355, 365, 375) `bg-rd-neutral-50` → `bg-white`. **Section dividers:** Her panelin alt blokları arasına net divider — bottom feature check ve mavi info kutu üstüne `pt-5 mt-5 border-t border-rd-neutral-200` (mevcut zaten var olabilir, doğrula). CTA blok: `mt-6 pt-5 border-t border-rd-neutral-200` (alanları belirginleştir). |

#### P8-1~P8-5 Birleşik Prompt
...
font-bold (H1, H2) ZORUNLU. text-[9-11px] yasak.
text-slate-* token'ları rd-* ile değiştir (Hero ve TrustStrip dahil).

Branch: claude/redesign-modern-ui (devam et)
Görev: P8-1~P8-5 — Polish-8, 5 madde tek pakette

ARKAPLAN:
P7 sonrası Aziz preview test:
- "yzliste Beta" logosu kalksın, normal logo gelsin
- /uret sticky bar üretim yokken "1 kredi" diyor, anlamsız
- Anasayfa başlık fontları/renkleri tutarsız (SectionHeader extrabold, Hero slate)
- Marka tonu 3 metni belirgin değil, premium markaDolu ile örtüşüyor
- InfoStrip detay alanı düz renk, alanlar belli değil

GÖREVLER (sırayla, ayrı commit'ler):

═══════════════════════════════════════════════════════════════════
P8-1: Logo "Beta" badge kaldır
═══════════════════════════════════════════════════════════════════

1. lib/constants/hero.ts:
   NAV_BRAND.betaBadge: true → false

2. components/SiteHeader.tsx ve components/sections/HeroBlock/Nav.tsx
   içinde "Beta" yazılı badge render eden span/div bul ve kaldır.
   
   Örnek: Eğer şöyle bir yapı varsa:
     <span className="...beta...">Beta</span>
   Bu satır ve etrafındaki conditional kaldırılsın.

3. Logo image (/yzliste_logo.png) DOKUNMA. Sadece kod tarafındaki
   text-based "Beta" badge'i kaldırılsın.

Commit: refactor(brand): P8-1 yzliste Beta badge kaldırıldı

═══════════════════════════════════════════════════════════════════
P8-2: /uret StickySubmitBar cost=0 fix
═══════════════════════════════════════════════════════════════════

1. components/uret/useCalculateCredits.ts:
  ...
  interface CalculateCreditsArgs {
    ...
    productName?: string         // YENİ
    hasPhoto?: boolean           // YENİ
  }
  ...
  export function calculateCredits(args: CalculateCreditsArgs): number {
    ...
    if (activeTab === 'metin') {
      // Önce ürün adı veya fotoğraf gerekli
      if (productName.trim() === '' && !hasPhoto) return 0
      return 1
    }
    if (activeTab === 'gorsel') return selectedStylesCount  // Math.max kaldır
    if (activeTab === 'video') {
      if (!hasPhoto) return 0  // Fotoğraf yoksa video yok
      return videoLengthSec === 10 ? 20 : 10
    }
    if (activeTab === 'sosyal') return selectedPlatformsCount  // Math.max kaldır
    return 0
  }

2. app/uret/page.tsx satır 137 calculateCredits çağrısı:
  ...
  const cost = calculateCredits({
    ...
    selectedPlatformsCount: 1, // mevcut, ne ise koru
    productName: metin.urunAdi,        // YENİ
    hasPhoto: fotolar.length > 0,      // YENİ
  })

3. components/uret/StickySubmitBar.tsx:

   Sol kolon (Cost summary, satır 32-50). cost === 0 senaryosu:

  <div className="flex-1">
    <p className="text-xs font-medium text-rd-neutral-500 uppercase tracking-wider">
      {cost === 0 ? 'Maliyet' : 'Bu üretimin maliyeti'}
    </p>
    <div className="mt-1 flex items-baseline gap-2">
      {cost === 0 ? (
        <>
          ...
          <span className="text-xs text-rd-neutral-500">
...
Commit: feat(uret): P8-2 StickySubmitBar cost=0 durumu (—) + form gerekli

═══════════════════════════════════════════════════════════════════
P8-3: Anasayfa tipografi tutarlılık
═══════════════════════════════════════════════════════════════════

1. components/primitives/SectionHeader.tsx satır 58-67:
...
2. components/sections/HeroBlock/HeroContent.tsx:

   Tüm text-slate-* → text-rd-neutral-*:
     text-slate-900 → text-rd-neutral-900
     text-slate-700 → text-rd-neutral-700
     text-slate-600 → text-rd-neutral-600
     text-slate-500 → text-rd-neutral-500

   border-slate-* → border-rd-neutral-*:
     border-slate-200 → border-rd-neutral-200
     border-slate-300 → border-rd-neutral-300

   bg-slate-* → bg-rd-neutral-*:
     bg-slate-50 → bg-rd-neutral-50

   Hero h1:
     font-[800] → font-bold
     style={{ fontSize: 'clamp(40px, 5.5vw, 64px)' }} korunsun (responsive sizing)
     leading-[1.05] tracking-[-0.025em] korunsun

   Arbitrary text size:
     text-[13px] → text-sm
     text-[15px] → text-base

3. components/sections/HeroBlock/TrustStrip.tsx:
   border-slate-200 → border-rd-neutral-200

4. Diğer dosyalarda text-slate/border-slate kullanımı varsa (grep yap, anasayfa kapsam):
   components/sections/HeroBlock/Nav.tsx
   components/sections/HeroBlock/AppScreenshotMockup.tsx
   components/sections/HeroBlock/HeroContent.tsx
   
   Hepsinde slate-* → rd-neutral-* swap.

Commit: refactor(landing): P8-3 anasayfa tipografi tutarlılık (slate→rd, extrabold→bold)

═══════════════════════════════════════════════════════════════════
P8-4: MarkaBilgileri 3 ton metni yenile + UX clarification
═══════════════════════════════════════════════════════════════════

1. lib/constants/marka-bilgileri.ts TONE_CHIPS array yenisi:
...
2. components/sections/MarkaBilgileriSection.tsx sol kolon — UX clarification:
   
   MB_HEADER subtitle paragrafından SONRA, "8 alan listesi"nden ÖNCE:
   ...
   <div className="mt-4 p-3 rounded-lg bg-rd-primary-50 border border-rd-primary-200">
     <p className="text-xs text-rd-primary-700 leading-relaxed">
       ...
       aynı bilgiler, farklı ses. <strong className="font-semibold">Marka bilgisi</strong> 
       içeriğe ne kadar kişisel detay katar — mağaza adı, hedef kitle, hizmet vurguları.
     </p>
   </div>

Commit: feat(marka): P8-4 ton metinleri yenilendi + UX clarification

═══════════════════════════════════════════════════════════════════
P8-5: InfoStrip görsel hiyerarşi — KRİTİK (alanlar belli olsun)
═══════════════════════════════════════════════════════════════════

components/landing/InfoStrip.tsx

1. Detay alanı container (satır ~502):
   
   Eski:
     className="mt-4 rounded-xl border border-rd-neutral-200 bg-rd-neutral-50 p-4 md:p-5"
   
   Yeni:
     className="mt-4 rounded-xl border border-rd-neutral-200 bg-rd-neutral-100 p-5 md:p-7"

   (bg-rd-neutral-50 → bg-rd-neutral-100, p-4 md:p-5 → p-5 md:p-7)

2. İç kartların background — TÜMÜNÜ bg-white yap:

   MetinPanel bölüm kartları (satır ~150):
     'rounded-xl border-l-4', bolum.renk, 'border border-rd-neutral-200 bg-rd-neutral-50 p-4'
     →
     'rounded-xl border-l-4', bolum.renk, 'border border-rd-neutral-200 bg-white p-4'

   VideoPanel 6 video kartı (satır ~281):
     "flex gap-3 rounded-xl border border-rd-neutral-200 bg-rd-neutral-50 p-3"
     →
     "flex gap-3 rounded-xl border border-rd-neutral-200 bg-white p-3"

   VideoPanel 3 özellik kartı (satır ~308):
     "rounded-xl border border-rd-neutral-200 bg-rd-neutral-50 p-3 text-center"
     →
     "rounded-xl border border-rd-neutral-200 bg-white p-3 text-center"

   VideoPanel "Nasıl çalışır" (satır ~317):
     "bg-rd-neutral-100 rounded-xl p-4"
     →
     "bg-white border border-rd-neutral-200 rounded-xl p-4"

   SosyalPanel 3 caption kartı (satır ~355, ~365, ~375):
     "rounded-xl border border-rd-neutral-200 bg-rd-neutral-50 p-4"
     →
     "rounded-xl border border-rd-neutral-200 bg-white p-4"

3. CTA bottom blok (her 4 panelin altında):
   
   Mevcut: <div className="mt-5 pt-4 border-t border-rd-neutral-200 text-center">
   Yeni: <div className="mt-6 pt-5 border-t border-rd-neutral-200 text-center">
   
   (mt-5 → mt-6, pt-4 → pt-5 — biraz daha nefes)

4. Bottom feature check ve mavi info bloğu (sadece MetinPanel'de):
   
   Mevcut yapısını koru ama section divider'ları açıkça doğrula:
     <div className="mt-5 pt-4 border-t border-rd-neutral-200 ...">
   
   border-t-rd-neutral-200 — bu beyaz kart üzerinde net görünsün. Eğer 
   detay container bg-rd-neutral-100 olduysa kontrast yeterli.

Test: Detay açıldığında her alt blok (intro paragraph, platform tabs, 
4 bölüm kartı, bottom feature, mavi info, CTA) net görsel olarak 
ayırt edilebilmeli. Beyaz kartlar zemin (rd-neutral-100) üzerinde 
yüzer görünmeli.

Commit: feat(landing): P8-5 InfoStrip görsel hiyerarşi (kart bg-white + zemin neutral-100)

═══════════════════════════════════════════════════════════════════

ACCEPTANCE (Code kendi yapsın bitiş öncesi):
[ ] "Beta" badge SiteHeader/Nav'da yok (i