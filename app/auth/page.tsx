...
export default function AuthPage() {
  ...
  const turkceHata = (hata: string): string => {
    ...
    if (hata.includes("Email not confirmed")) return "E-posta adresinizi doğrulayınız.";
    if (hata.includes("User already registered")) return "Bu e-posta adresi zaten kayıtlı.";
    if (hata.includes("invalid") && hata.includes("email")) return "Geçerli bir e-posta adresi giriniz.";
    if (hata.includes("rate limit") || hata.includes("too many")) return "Çok fazla deneme yapıldı. Lütfen biraz bekleyin.";
  ...
  const handleSubmit = async () => {
    ...
    if (mod === "kayit" && !sozlesmeOnay) { setMesaj("Devam etmek için sözleşmeleri kabul etmelisiniz."); return; }
    setYukleniyor(true);
    setMesaj("");
    if (mod === "kayit") {
  ...
  const modalUyeGiris = async () => {
    ...
    if (!modalSifre.trim()) { setModalMesaj("Şifre giriniz."); return; }
    if (modalUyeMod === "kayit" && !modalSozlesme) { setModalMesaj("Sözleşmeleri kabul etmelisiniz."); return; }
  ...
  const odemeBaslat = async (paket: string) => {
    ...
    try {
      ...
      if (data.checkoutFormContent) {
        ...
        setModalMesaj(data.hata || "Ödeme başlatılamadı.");
      }
    } catch {
      setModalMesaj("Bir hata oluştu, tekrar deneyin.");
    }
    ...
    <main className="min-h-screen bg-white font-sans">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            {modalMod === "uye" ? (
              <div className="p-6 space-y-4">
                <p className="text-sm text-gray-500">Devam etmek için hesabınıza giriş yapın veya ücretsiz kayıt olun.</p>
                ...
                {modalUyeMod === "kayit" && (
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <span className="text-xs text-gray-500 leading-relaxed">
                      <a href="/gizlilik" target="_blank" className="text-orange-500 hover:underline">Gizlilik Politikası</a> ve <a href="/mesafeli-satis" target="_blank" className="text-orange-500 hover:underline">Mesafeli Satış Sözleşmesi</a>'ni okudum, kabul ediyorum.
                    </span>
                  </label>
                )}
      ...
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-gray-100 px-6 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => { handleModDegistir("giris"); document.getElementById("auth-form")?.scrollIntoView({ behavior: "smooth" }); }} className="text-sm text-gray-500 hover:text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-100 transition-colors">Giriş Yap</button>
            <button onClick={() => { handleModDegistir("kayit"); document.getElementById("auth-form")?.scrollIntoView({ behavior: "smooth" }); }} className="text-sm bg-orange-500 text-white px-4 py-2 rounded-lg hover:bg-orange-600 transition-colors font-medium">Ücretsiz Başla</button>
          </div>
        </div>
      </header>
      ...
      <section className="px-6 pt-16 pb-10 text-center max-w-3xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <button onClick={() => { handleModDegistir("kayit"); document.getElementById("auth-form")?.scrollIntoView({ behavior: "smooth" }); }} className="bg-orange-500 hover:bg-orange-600 text-white font-semibold px-8 py-4 rounded-xl text-base transition-colors shadow-lg shadow-orange-100">3 Ücretsiz Hakla Başla →</button>
          <button onClick={() => document.getElementById("nasil-calisir")?.scrollIntoView({ behavior: "smooth" })} className="text-gray-500 hover:text-gray-800 font-medium px-8 py-4 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors text-base">Nasıl çalışır?</button>
        </div>
      </section>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-orange-50">
              ...
              <button onClick={() => setOrnekAcik(!ornekAcik)} className="text-xs text-orange-600 hover:text-orange-800 font-medium">{ornekAcik ? "Küçült ▲" : "Tümünü gör ▼"}</button>
            </div>
        ...
        <div className="max-w-md mx-auto">
          <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-8">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-1">{mod === "kayit" ? "Ücretsiz hesap oluştur" : "Tekrar hoş geldin"}</h2>
              <p className="text-sm text-gray-400">{mod === "kayit" ? "3 ücretsiz kullanım hakkı hediye" : "Hesabına giriş yap"}</p>
            </div>
            <div className="space-y-3">
              <input type="email" placeholder="E-posta" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent" />
              <input type="password" placeholder="Şifre" value={sifre} onChange={(e) => setSifre(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSubmit()} autoComplete={mod === "kayit" ? "new-password" : "current-password"} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent" />
              {mod === "kayit" && (
                <label className="flex items-start gap-3 cursor-pointer mt-1">
                  <input type="checkbox" checked={sozlesmeOnay} onChange={(e) => setSozlesmeOnay(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-gray-300 text-orange-500 focus:ring-orange-400 cursor-pointer flex-shrink-0" />
                  <span className="text-xs text-gray-500 leading-relaxed">
                    <a href="/gizlilik" target="_blank" className="text-orange-500 hover:underline font-medium">Gizlilik Politikası</a>{" "}ve{" "}
                    <a href="/mesafeli-satis" target="_blank" className="text-orange-500 hover:underline font-medium">Mesafeli Satış Sözleşmesi</a>{" "}ile{" "}
                    <a href="/teslimat-iade" target="_blank" className="text-orange-500 hover:underline font-medium">Teslimat ve İade Şartları</a>'nı okudum ve kabul ediyorum.
                  </span>
                </label>
              )}
              {mesaj && <p className={`text-xs ${mesaj.includes("başarılı") ? "text-green-600" : "text-red-500"}`}>{mesaj}</p>}
              <button onClick={handleSubmit} disabled={yukleniyor || (mod === "kayit" && !sozlesmeOnay)} className="w-full bg-orange-500 hover:bg-orange-600 disabled:bg-gray-300 text-white font-semibold py-3 rounded-xl transition-colors text-sm">
