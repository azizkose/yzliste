...
**D-01: "Tahmini Tasarruf" kartını kaldır veya düzelt (P2)**
`~0sa` gösteriyor — 17 üretim yapılmış ama saat hesabı sıfır. Formül: `toplam_kullanilan * 0.5`. Ya `toplam_kullanilan` DB'de sıfır ya da hesap kırık. Bu kart anlamsız görünüyor → ya düzelt (üretim başına 15dk tasarruf varsayımıyla `uretimSayisi * 0.25` saat yap) ya da bu kartı "Toplam Üretim" ile değiştir (tüm zamanların toplam üretimi — daha somut).
**Dosya:** `app/(auth)/hesap/page.tsx` — 4. metrik kartı

**D-02: Kredi 0 olduğunda dashboard'a CTA banner ekle (P1)**
Kredi bittiğinde dashboard sessiz kalıyor. Kullanıcı "kredilerim bitti, ne yapayım" diye düşünüyor ama aksiyon yok.
**Fix:** Kredi ≤ 0 olduğunda kartların üstüne belirgin bir banner:
```tsx
{kredi <= 0 && (
  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-center justify-between">
    <div>
      <p className="font-semibold text-amber-800">Kredilerin bitti!</p>
      <p className="text-sm text-amber-600">Üretim yapmaya devam etmek için kredi yükle.</p>
    </div>
    <a href="/hesap/krediler" className="bg-amber-500 text-white px-5 py-2 rounded-lg font-semibold hover:bg-amber-600 shrink-0">
      Kredi Yükle →
    </a>
  </div>
)}
```
**Dosya:** `app/(auth)/hesap/page.tsx` — kartların üstüne

**D-03: Platform dağılımı göster (P2)**
"Favori Platform: Trendyol" tek satırı yeterli değil. Tüm platformların kullanım oranını küçük bir yatay bar ile göster.
**Fix:** Mevcut `uretimler` sorgusundan platform count'ları zaten çekiliyor (favori hesaplama için). Bu veriyi bar olarak göster:
```tsx
// Örnek: platformlar = { Trendyol: 10, "Amazon TR": 4, Etsy: 3 }
// Her biri için yatay bar + sayı
{Object.entries(platformlar).sort((a,b) => b[1] - a[1]).map(([p, sayi]) => (
  <div key={p} className="flex items-center gap-2 text-sm">
    <span className="w-20 text-gray-600">{p}</span>
    <div className="flex-1 bg-gray-100 rounded-full h-2">
      <div className="bg-indigo-400 rounded-full h-2" style={{width: `${(sayi/toplamUretim)*100}%`}} />
    </div>
    <span className="text-gray-400 text-xs">{sayi}</span>
  </div>
))}
```
"Favori Platform" kartını bu breakdown ile değiştir veya altına ekle.
**Dosya:** `app/(auth)/hesap/page.tsx`

**D-04: Son üretimlere "Tümünü Gör" linki ekle (P2)**
Şu an sadece son 3 üretim gösteriliyor, daha fazlasına erişim yok.
**Fix:** Son üretimler bölümünün altına:
```tsx
<a href="/hesap/gecmis" className="text-sm text-indigo-500 hover:underline">Tüm üretim geçmişini gör →</a>
```
`/hesap/gecmis` mevcut değilse `/profil` sayfasındaki geçmiş üretimler bölümüne yönlendir (varsa).
**Dosya:** `app/(auth)/hesap/page.tsx` — son üretimler bölümü sonrası

**D-05: Krediler sayfasında paket bilgisi göster (P2)**
Kullanıcı hangi paketi aldığını göremez. Ödeme geçmişinden çıkarılabilir ama ayrı bir kart olarak göstermek daha iyi.
**Fix:** Ödeme geçmişinin üstüne son satın alınan paketi göster:
```tsx
// Son ödeme kaydından paket adını çek
<div className="bg-indigo-50 rounded-xl p-4 mb-6">
  <p className="text-xs text-indigo-500 font-semibold">Son Paketiniz</p>
  <p className="font-bold text-indigo-800">{sonPaket.paketAdi} — {sonPaket.kredi} kredi</p>
  <p className="text-xs text-indigo-400">{sonPaket.tarih} tarihinde alındı</p>
</div>
```
**Dosya:** `app/(auth)/hesap/krediler/page.tsx`
